...
int frame_add_tag( uint32_t tag, void* data, int data_len ) {
    ...
    if ( frame.n_bytes + total_length > max_frame ) return -1;
...
static int frame_append_data( void* data, int data_len ) {
    if ( frame.n_bytes + data_len > max_frame ) return -1;
...
int frame_append_packet( void* data, int data_len ) {
    if ( frame.n_packets >= max_packets_per_frame ) return -1;

    int expected_adth_size = sizeof( struct next_header ) + 4 + ( frame.n_packets + 1 ) * sizeof( struct bounds );

    if ( frame.n_bytes + data_len + 16 + expected_adth_size > max_frame ) return -1;
...
static void handle_mux_frame( int mux, int tun ) {
    ...
    for ( int i = 0; i < n_packets; i++ ) write( tun, &inbuf[ bounds[ i ].offset ], bounds[ i ].length );
}
...
static void handle_tun_frame( int tun, int mux ) {
    ...
    if ( ret ) ret = frame_append_packet( inbuf, count );
...
int main( int argc, char** argv ) {
    ...
    while ( 1 ) {
        ...
        if ( frame.n_packets ) {
            ...
            if ( now.tv_sec >= frame.deadline.tv_sec && now.tv_nsec >= frame.deadline.tv_nsec ) goto frame_out;
        ...
        if ( FD_ISSET( mux, &fds ) ) handle_mux_frame( mux, tun );

        if ( FD_ISSET( tun, &fds ) ) handle_tun_frame( tun, mux );
